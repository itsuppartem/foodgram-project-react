...
const SingleCard = ({ loadItem, updateOrders }) => {
  ...
  return <Main>
    <Container>
      <MetaTags>
        <title>{name}</title>
        <meta name="description" content={`Grocery Assitant - ${name}`} />
      ...
      <div className={styles['single-card']}>
        ...
        <div className={styles["single-card__info"]}>
          ...
          <div className={styles['single-card__buttons']}>
            ...
            {is_in_shopping_cart ? <><Icons.DoneIcon color="#a23eff"/>Recipe has added</> : <><Icons.PlusIcon /> Add to buying list</>}
            </Button>}
            ...
            >
              {author.is_subscribed ? 'Subscribe to the author' : 'Unsubscribe from the author'}
            </Button>}
          </div>
